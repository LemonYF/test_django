...
def add_event(request):
    ...
    result = Event.objects.filter(neme=name)
    if result:
...
def get_event_list(request):
    ...
    if name != '':
        ...
        if results:
            for r in results:
                event = {'name': results.name, 'limit': results.limit, 'status': results.status,
                         'address': results.address, 'start_time': results.start_time}
                datas.append(event)
...
def add_guest(request):
    ...
    if eid == '' or realname == '' or phone == '':
        return JsonResponse({'status': 10021, 'message': 'para error'})
    ...
    gusit_limit = Event.objects.filter(event_id=eid)

    if len(gusit_limit) > event_limit:
